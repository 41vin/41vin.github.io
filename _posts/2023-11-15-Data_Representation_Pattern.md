...
텍스트와 관련된 auxiliary learning task은 Word2Vec과 BERT가 있다.

- Word2Vec:
  - CBOW와 Skip-gram 기술을 결합하여 사용된다.
  - CBOW
    - 문맥 안에 있는 주변 단어들을 가지고 특정 단어를 예측하는 방식으로 학습
  - Skip-gram
    - 특정 단어를 가지고 주변 단어를 예측하는 방식으로 학습
  - 모델의 목표
    - 입력 단어와 대상 단어 간의 관계를 학습하는 것이지만, low dimension embedding을 학습하여 단어의 문맥을 잘 포착할 수 있게 한다.
- BERT
  - Transformer 아키텍쳐를 기반 모델이다.
  - Masked Language Model(MLM)이라는 학습 작업을 사용하여, 주어진 문장에서 일부 단어를 가리고 그 가려진 단어를 예측하도록 모델을 학습한다.
  - BERT는 위와 같은 과정을 통해 문맥을 이해하고 문장 내의 단어 간 상호작용을 파악할 수 있다.
...
- Need for regularization
  - 매우 미세한 bucket을 사용한다면 feature cross가 매우 정확하여 성능이 좋아진다. 하지만 cardinality가 높아진다는 단점이 있다. 이러한 이유로 인해 Feature cross를 수행한 후 feature의 density를 높이는 L1 regularization 또는 overfitting을 제한하는 L2 regualrization을 함께 쓰는 것이 좋다.
  - Feature cross를 위해 결합할 feature를 선택할 때 correlation이 높은 두 feature를 선택하는 것은 권장되지 않는다.

---

## Design Pattern 4: Multimodal Input

Multimodal Input 디자인 패턴은 사용 가능한 모든 데이터 표현을 연결하여, 복잡한 방식으로 표현할 수 있는 데이터 또는 다양한 유형의 데이터를 표현하는 문제를 해결한다.



### Problem

![image-20231115172238734](../assets/img/2023-11-15-Data_Representation_Pattern/image-20231115172238734.png)

모델은 Image feature와 Numerical feature를 모두 처리해야 한다. 수치 데이터와 달리 이미지나 텍스트는 모델에 직접 공급할 수 없다. 그래서 모델이 이해할 수 있는 방식으로 이미지 및 텍스트 입력을 표현한 다음(embedding 사용) 이러한 입력을 다른 feature와 결합해야 한다.



### Solution

![image-20231115172849270](../assets/img/2023-11-15-Data_Representation_Pattern/image-20231115172849270.png)

meal-type은 세가지 카테고리를 가질 수 있으므로, 이를 one-hot encoding으로 변환해서 저녁 식사를 [0, 0, 1]과 같이 나타낸다. 배열을 이용해 feature를 조합하면 [0, 0, 1, 30,5]와 같이 결합할 수 있다.



### Trade-Offs and Alternatives

Multimodal 입력은 다음의 두 가지를 모두 의미한다.
...
평점을 모델의 입력으로 넣을 때, 1~5까지 5개의 bucket과 (좋음, 나쁨)으로 2개 bucket으로 동시에 입력 받을 수 있다. 여기서 만들어지는 feature는 정수로 된 평점과 boolean으로 구성된 2개의 원소를 가지는 배열이다.  



- Text data multiple ways

  - BOW는 텍스트의 순서를 유지하지는 않지만 모델에 봰는 각 텍스트 조각에서 특정 단어의 존재 여부를 감지한다. 이 방식을 쓰면 각 텍스트 입력이 1과 0의 배열로 변환되는 multi hot-encoding 유형이 된다.

  - Embedding은 모델에 계층을 추가하고 BOW 인코딩에서 알 수 없는 단어 의미에 대한 정보를 제공해 줄 수 있다. 하지만 embedding은 학습이 필요하다는 단점이 있다.
  - BOW 인코딩은 Scikit-learn, XGboost와 같은 딥러닝 프레임워크와 결합되어 높은 정확도를 달성할 수 있다.



- Multimodal representation of images
  - Image as pixel values
    - 이미지를 픽셀값의 배열로 표현하는 방식은 MNIST 데이터셋의 흑백 이미지와 같은 단순한 이미지에 대해서는 잘 작동하지만 이미지가 복잡해진다면 잘 작동하지 않는다.



- Image as tiled structures

  - CNN
    - 목표: 현실적인 이미지의 세부사항을 추출하고 이미지 내의 패턴을 이해할 수 있도록 표현하는 것
    - 사용 이유: 
      - CNN은 이미지의 작은 영역을 한 번에 처리할 수 있어 모델이 이웃하는 픽셀 사이의 공간적 gradient와 edge를 식별할 수 있다.
      - Convolution layer를 사용함으로써 CNN은 이미지에서 계층적 특징을 포착할 수 있으며, 가장 단순한 특징인 edge 부터 복잡한 표현까지 모두 점진적으로 구축할 수 있다.
      - 이미지의 작은 부분을 처리하는 것은 모델이 픽셀 간의 공간적인 관계를 더 잘 이해하게 해준다. 이는 패턴을 인식하고 이미지로부터 의미 있는 정보를 추출하는데 중요하다.

  - 일반적으로 이미지가 자세할수록 이미지를 타일로 표현하는 것이 좋다. Pixel과 Tile 두 가지 방법을 결합하여 더 높은 정확도를 얻을 수 있다.
